...
export default class MainText extends React.Component {
	...
	render() {
		...
		for (var i=0; i<this.state.ANNOTATIONS.length; i++){
			var current = (<AnnotationModal key={i} desc={this.state.ANNOTATIONS[i].description}></AnnotationModal>);
			modals.push(current);
			spans.push(<AnnotationButton key={i} modal={current}>hello</AnnotationButton>)
		}
		var a = <AnnotationButton key={i} modal={current}>hello</AnnotationButton>;
		return (
			<div>
